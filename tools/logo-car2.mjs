...
for (const th of [18, 24, 30]) {
  // car stroke is DARKER than navy bg: keep pixels below th as black
  const buf = await sharp(SRC)
    .resize(W, H, { kernel: "lanczos3" })
    .extract({ left: 0, top: 0, width: W, height: Math.round(H * 0.64) })
    .grayscale().threshold(th).png().toBuffer();
  ...
  fs.writeFileSync(path.join(OUT, `logo-car-t${th}.svg`), clean);
  ...
  fs.writeFileSync(path.join(OUT, `logo-car-t${th}-preview.png`), preview);
  console.log("threshold", th, "bytes", clean.length);
}
